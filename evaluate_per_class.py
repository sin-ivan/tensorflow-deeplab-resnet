...
from iou_per_class import streaming_mean_iou_per_class
...
def main():
    ...
    IoU, mIoU, update_op = streaming_mean_iou_per_class(
    ...
    for step in range(args.num_steps):
        ...
        iou_list.append(IoU.eval(session=sess))
    print('Mean IoU: {:.3f}'.format(mIoU.eval(session=sess)))
    coord.request_stop()
    coord.join(threads)

    with open('iou_list.pkl', 'wb') as f:
        pickle.dump(iou_list, f)
    iou_list_np = np.array(iou_list)
    iou_per_class = np.mean(iou_list_np, axis=0)
