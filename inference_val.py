"""Inference script for the DeepLab-ResNet network on the validation subset
...
def main():
    ...
    # Load image index list
    image_index_list = []
    with open(args.data_list, 'r') as f:
        image_index_list = f.readlines()
        image_index_list = [os.path.splitext(os.path.basename(idx.split(' ')[0]))[0] for idx in image_index_list]
    ...
    for step in range(args.num_steps):
        ...
        im.save(os.path.join(args.save_dir, image_index_list[step] + '.png'))
        if step % 100 == 0:
